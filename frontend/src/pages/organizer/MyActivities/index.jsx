...
const STATUS_MAP = {
  ...
  2: { text: '已通过', color: 'success', icon: <CheckCircleOutlined /> },
  3: { text: '已驳回', color: 'error', icon: <ExclamationCircleOutlined /> },
  4: { text: '已发布', color: 'blue', icon: <CheckCircleOutlined /> },
  5: { text: '已取消', color: 'default', icon: <StopOutlined /> },
  6: { text: '已结束', color: 'default', icon: <CheckCircleOutlined /> },
  7: { text: '进行中', color: 'green', icon: <TeamOutlined /> }
}
...
function MyActivities() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        // 已通过/已发布状态：可查看报名、取消活动
        if (record.status === ACTIVITY_STATUS.APPROVED || record.status === ACTIVITY_STATUS.PUBLISHED) {
          actions.push(
  ...
  const getStatCards = () => {
    const stats = {
      ...
      published: activities.filter(a => a.status === 4 || a.status === 2).length,
      inProgress: activities.filter(a => a.status === 7).length
    }
    return stats
  }
  ...
  const tabItems = [
    ...
    { key: '4', label: `已发布 (${stats.published})` },
    { key: '7', label: `进行中 (${stats.inProgress})` }
  ]
